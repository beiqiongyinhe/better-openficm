...
- 主要改动见文末「[本修改版相对上游的改动](#本修改版相对上游的改动)」。

> ⚠️ **本版签名证书与上游官方版不同**：`ff68431a9d797589a0f6034848dc1c34ae4e3acba878d5183b5afe941f08a837`
> 已安装上游官方版的设备**无法直接覆盖安装本版**，需先卸载官方版。卸载会清空本地作品数据，请务必先导出。
...
本仓库在其之上做了以下增强（`git diff upstream/main`，27 个文件，+1908 / −247，其中新增 4 个文件）。

### 新增：从手机本地导入作品

上游的作品页只能手动新建空白作品、再一章章粘贴正文。本版把「导入」做成了正经功能：在作品页顶部点「**新建或导入作品**」，直接选手机里的文件即可。

- 支持 **TXT / Markdown / EPUB** 三种格式，单文件上限 50 MB
- 导入时按**卷 / 章结构**解析，而不是按字数硬切：
  - EPUB 读书脊段落与页面边界，卷章边界最准
  - TXT / Markdown 按标题层级识别
- 完成后提示「已导入《X》，共 N 章」
- 作品名优先取文档内标题，其次用文件名，兜底「未命名作品」

**实测结果**（真机数据库，三本长篇）：

| 作品 | 卷数 | 章数 | 结果 |
| --- | --- | --- | --- |
| 遭银龙王女变成幼龙姬的我 | 9 | 774 | 卷名逐字正确，各卷 100/144/114/76/76/104/99/28/33 |
| 恶役少爷怎么可能会是圣女？ | 2 | 270 | 正确 |
| 她们认清感情，骑士却已转生幼狐 | 4 | 317 | 正确 |

三本均**无空章、无重复章、无「第一章（1）」式碎片命名**。

> 说明：原文自带的作者公告类短章（「说明」「已更改」等）不会被自动过滤，以免误切真实章节。

### 新增：作品封面

- 可为每部作品挑一张封面，图片复制进应用私有目录保存
- 换封面时自动清理旧图，不留残留
- 未设封面时回退成书名首字占位图
- 老版本作品库自动补列升级，**已有作品与正文不丢**

### 新增：作品整理

- **编辑** — 直接改作品名与简介
- **排序** — 在作品列表里上下移动，按自己的习惯排
- **合并** — 把一部作品并入另一部（合并前会明确提示：被并入方将被删除，其章节、角色、世界书与笔记不会自动迁移）

### 新增：助手对话附件

- 对话里可直接附带手机文件，正文会真正进入模型上下文
- 单文件上限 50 MB，提取正文上限 800 万字符
- 超长附件进入提示词时按 20000 字符截断，并标注「（附件过长，已截断）」

### 助手页体验优化

- **长回复折叠** — 超过 600 字的消息默认折叠为 12 行，可一键展开 / 收起
- **对话进度条** — 右侧拖动，快速跳到任意位置
- **回到界面底部** — 滚动离开底部后出现按钮，一键回底
- **编辑重发** — 修改已发送的消息，会从该处重开分支

### 设置页增强

- **模型参数可编辑 / 删除** — 直接改已有模型的名称、模型 ID、温度、最大输出，或删掉不再用的配置
  - 温度需在 0–2 之间；最大输出需为整数（提示：1M 通常是上下文窗口，不要填成 1000000）
  - 修改不会影响当前选中的模型
- **分类页返回键接管** — 进入设置分类页后，按系统返回键退回设置列表，而不是直接退出 App（目前只接管硬件返回键，手势侧滑返回尚未覆盖）

### 其他

| 类别 | 说明 |
| --- | --- |
| 文档解析统一 | TXT / Markdown / EPUB 的提取逻辑收敛到 `lib/text-extract.ts` 一个模块；`style/source-library.ts` 中重复的一套实现（含重复的大小 / 字数上限常量）被移除，该文件从 414 行降到 231 行 |
| 图标 / 启动图 | 重做应用图标与启动图，体积显著缩小 |
| 依赖锁 | `package-lock.json` 同步更新，`.gitignore` 补充忽略项 |
