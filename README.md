...
本仓库在其之上做了以下修改（`git diff upstream/main`，25 个文件，+1811 / −233）：

### 修复：小说导入的章节切分缺陷（主要改动）

此前导入长篇小说时，章节被**硬切**成了「第一章（1）」「第一章（2）」……「（68）」这样的碎片，正文割裂、卷结构丢失。

现改为按**卷 / 章结构**解析后再落库：

- 新增 `mobile-rn/src/lib/import-split.ts` — 卷章切分核心
- 新增 `mobile-rn/src/lib/novel-import.ts` — 导入流程编排
- 改写 `mobile-rn/src/lib/text-extract.ts` — 文本提取与标题识别
- 改写 `mobile-rn/src/data/repositories.ts` — 支持按卷批量写章
...
三本均**无空章、无重复章、无「（1）（2）」式碎片命名**。

> 说明：原文自带的作者公告类短章（「说明」「已更改」等）不会被自动过滤，以避免误切真实章节。

### 其他改动

| 类别 | 文件 |
| --- | --- |
| 附件读取 | `lib/attachments.ts`（新增）、`agent/runtime.ts` |
| 项目页改造 | `screens/projects-screen.tsx` |
| 助手页调整 | `screens/assistant-screen.tsx` |
| 设置页调整 | `screens/settings-screen.tsx` |
| 数据层扩展 | `data/database.ts`、`types.ts` |
| 风格库精简 | `style/source-library.ts`（−193 行） |
| 图标 / 启动图 | `assets/*.png`、`android/**/splashscreen_logo.png` |
